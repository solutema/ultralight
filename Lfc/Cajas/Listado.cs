        ...
        "Por Día|dia"};
                ...
                public override Lfx.Types.OperationResult RefreshList()
                {
                        ...
                        if (mostrarReturn.Success == true) {
                                ...
                                TextoSql = "SELECT cajas_movim.* FROM cajas_movim";

                                if (m_TipoConcepto > 0)
                                        TextoSql += " LEFT JOIN conceptos ON cajas_movim.id_concepto=conceptos.id_concepto";

                                TextoSql += " WHERE (TRUE";
                                if (m_Caja > 0)
                                        TextoSql += " AND cajas_movim.id_caja = " + m_Caja.ToString();

                                TextoSql += " AND cajas_movim.fecha BETWEEN '" + sFecha1 + " 00:00:00' AND '" + sFecha2 + " 23:59:59'";
                                if (m_Concepto > 0)
                                        TextoSql += " AND cajas_movim.id_concepto=" + m_Concepto.ToString();

                                if (m_Persona > 0)
                                        TextoSql += " AND cajas_movim.id_cliente=" + m_Persona.ToString();

                                if (m_TipoConcepto > 0)
                                        TextoSql += " AND conceptos.grupo=" + m_TipoConcepto.ToString();


                                if (m_Direccion == 1)
                                        TextoSql += " AND cajas_movim.importe>=0";
