...
namespace Lbl.Cupones
{
	...
	public class Cupon : ElementoDeDatos
	{
		...
		public Cupon(Lfx.Data.DataBase dataBase, double importe, Pagos.FormaDePago tarjeta, Cupones.Plan plan, string numero, string autorizacion)
			: this(dataBase)
		{
			Importe = importe;
			FormaDePago = tarjeta;
		...
		public override string CampoId
		{
			get
			{
                ...
                public bool Anulado
                {
                        get
                        {
                                return this.Estado == ((int)(Lbl.Cupones.Estado.Anulado));
                        }
                }
                ...
                public override void OnLoad()
                {
                        if (this.Registro != null) {
                                if (Lfx.Data.DataBase.ConvertDBNullToZero(Registro["id_tarjeta"]) > 0)
                                        this.FormaDePago = new Pagos.FormaDePago(this.DataBase, System.Convert.ToInt32(Registro["id_tarjeta"]));
                                else
