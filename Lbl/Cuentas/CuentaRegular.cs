...
namespace Lbl.Cuentas
{
	...
	{
		//Heredar constructor
		public CuentaRegular(Lws.Data.DataView dataView, int idCuenta)
			: base(dataView)
		{
			m_ItemId = idCuenta;
		}
		...
		{
			return this.DataView.DataBase.FieldDouble("SELECT saldo FROM cuentas_movim WHERE id_cuenta=" + m_ItemId.ToString() + " ORDER BY id_movim DESC");
		}

                
                public Lfx.Types.OperationResult Movimiento(bool auto, Lbl.Cuentas.Concepto concepto, string textoConcepto, Lbl.Personas.Persona cliente, double importe, string obs, Lbl.Comprobantes.ComprobanteConArticulos factura, Lbl.Comprobantes.Recibo recibo, string comprobantes)
                {
                ...
                public Lfx.Types.OperationResult Movimiento(bool auto, int idConcepto, string concepto, int idCliente, double importe, string obs, int idFactura, int idRecibo, string comprobantes)
		{
			double SaldoActual = this.Saldo();
                        ...
                        Lfx.Data.SqlTableCommandBuilder Comando; Comando = new Lfx.Data.SqlInsertBuilder(this.DataView.DataBase, "cuentas_movim");
			Comando.Fields.AddWithValue("id_cuenta", m_ItemId);
