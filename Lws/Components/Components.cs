...
namespace Lws.Components
{
	...
	public static class ComponentManager
	{
                ...
                public static Lfx.Types.OperationResult Run(Lws.Workspace workspace, System.Windows.Forms.Form parentForm, string componentName, string functionName)
                {
                        Lws.Components.Component ComponentObject;

                        if (ComponentesCargados.ContainsKey(componentName + "." + functionName)) {
                                //Ya fue cargado
                                ComponentObject = (Lws.Components.Component)ComponentesCargados[componentName + "." + functionName];
                        } else {
                                //Creo una nueva instancia
                                ComponentObject = Lws.Components.ComponentManager.LoadComponent(componentName, functionName);
                        ...
                        if (ComponentObject != null) {
                                ...
                                if (Result != null && Result is System.Windows.Forms.Form) {
                                        ...
                                        if (ComponentObject.ComponentType == Lws.Components.ComponentTypes.MdiChildren) {
                                                ResultForm.MdiParent = parentForm;
                                                ResultForm.Show();
                                        } else if (ComponentObject.ComponentType == Lws.Components.ComponentTypes.NormalWindow) {
                                                ResultForm.Show();
                                        }
                                }
		...
		public static Lws.Components.Component LoadComponent(string componentName)
		{
			return LoadComponent(componentName, componentName);
		}

		public static Lws.Components.Component LoadComponent(string componentName, string functionName)
		{
			string[] WhereToLook;
			...
			if(ComponentAssembly == null)
				...
				return (Lws.Components.Component)ComponentAssembly.CreateInstance(componentName + "." + functionName);
		}
	}
}
