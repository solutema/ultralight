...
namespace Lui.Printing
{
        public struct Par
        {
                ...
                public Par(string nombre, string valor, int nivel)
                {
                        this.Nombre = nombre;
                        this.Valor = valor;
                        this.Nivel = nivel;
        ...
        public class ItemPrint : System.Drawing.Printing.PrintDocument
        {
                private string m_Titulo = "Item";
                private int m_TamanoFuente;
                private Image m_Imagen;
                private List<Par> m_Pares;

                public void AgregarPar(string Nombre, string Valor, int Nivel)
                {
                        if (m_Pares == null)
                                m_Pares = new List<Par>();

                        Par NuevoPar = new Par(Nombre, Valor, Nivel);
                        m_Pares.Add(NuevoPar);
                }
                ...
                protected override void OnPrintPage(System.Drawing.Printing.PrintPageEventArgs ev)
                {
                        ...
                        foreach (Par TalPar in m_Pares) {
                                ...
                                SizeF Tamano1 = ev.Graphics.MeasureString(EstePar.Nombre, FuenteNombre, Rect.Size, FormatoLT);
                                Rect2 = new RectangleF(intMarginLeft + 168, iTop, intPrintAreaWidth - 168, 320);
                                SizeF Tamano2 = ev.Graphics.MeasureString(EstePar.Valor, Fuente, Rect2.Size, FormatoLT);
                                ...
                                if (EstePar.Valor.Length == 0) {
                                        EstePar.Valor = "-";
                                }

                                if (EstePar.Nombre == "-" && EstePar.Valor == "-") {
                                        iTop += System.Convert.ToInt32(Rect.Height + 4);
                                        // No imprimo nada
                                } else {
                                        ev.Graphics.DrawString(EstePar.Nombre, FuenteNombre, Brushes.Black, Rect, FormatoLT);
                                        ev.Graphics.DrawString(EstePar.Valor, Fuente, Brushes.Black, Rect2, FormatoLT);
                                        iTop += System.Convert.ToInt32(Rect.Height + 4);
